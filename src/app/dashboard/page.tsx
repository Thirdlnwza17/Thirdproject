  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-100 via-blue-300 to-blue-500 p-4">
      <div className="w-full max-w-6xl bg-white/90 rounded-3xl shadow-2xl mt-10 p-8 flex flex-col items-center border border-white/30 backdrop-blur-xl relative">
        ...
        {/* Attest Table Summary - New Design */}
        <div className="w-full mb-12">
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
            {/* Table Header */}
            <button 
              onClick={toggleAttestTable}
              className="w-full px-6 py-4 text-left hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors flex justify-between items-center"
              aria-expanded={isAttestTableExpanded}
            >
              <h3 className="text-xl font-bold text-sky-800 flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                </svg>
                สรุปผลรอบตรวจสอบ (attest_table)
              </h3>
              <svg 
                className={`w-5 h-5 text-gray-500 transform transition-transform ${isAttestTableExpanded ? 'rotate-180' : ''}`}
                fill="none" 
                viewBox="0 0 24 24" 
                stroke="currentColor"
              >
            ...
            {/* Collapsible Content */}
            <div 
              className={`px-6 pb-6 transition-all duration-300 ease-in-out ${isAttestTableExpanded ? 'block' : 'hidden'}`}
              aria-hidden={!isAttestTableExpanded}
            >
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Autoreader 490 Card */}
                <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  <h4 className="font-semibold text-blue-700 mb-3 text-center text-lg">เฉพาะ Autoreader 490 (SN 101715)</h4>
                  <div className="grid grid-cols-5 gap-2 text-sm font-medium text-gray-700 mb-2 px-2">
                    <div className="text-center">ช่อง</div>
                    <div className="text-center">Used</div>
                    <div className="text-center">ผ่าน</div>
                    <div className="text-center">ไม่ผ่าน</div>
                    <div className="text-center">อัตราผ่าน</div>
                  </div>
                  <div className="space-y-2">
                    {attestSummary490.map((s, idx) => (
                      <div key={idx} className="bg-white rounded-lg p-3 shadow-sm border border-gray-100 hover:bg-blue-50/50 transition-colors">
                        <div className="grid grid-cols-5 gap-2 items-center">
                          <div className="text-center font-bold text-gray-900">{idx + 1}</div>
                          <div className="text-center text-gray-700">{s.used}</div>
                          <div className="text-center text-green-600 font-medium">{s.pass}</div>
                          <div className="text-center text-red-600 font-medium">{s.fail}</div>
                          <div className={`text-center font-medium ${
                            s.passRate >= 90 ? 'text-green-700' : 
                            s.passRate >= 70 ? 'text-yellow-600' : 'text-red-700'
                          }`}>
                ...
                {/* Autoreader 390G Card */}
                <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  <h4 className="font-semibold text-blue-700 mb-3 text-center text-lg">เฉพาะ Autoreader 390G (SN 431930)</h4>
                  <div className="grid grid-cols-5 gap-2 text-sm font-medium text-gray-700 mb-2 px-2">
                    <div className="text-center">ช่อง</div>
                    <div className="text-center">Used</div>
                    <div className="text-center">ผ่าน</div>
